...
function resolveGSettingsValue(value, oldValue) {
    if (typeof(value) === 'object') {
        ...
        case 'append-to-list-unique':
            {
                // 'Set' is part of the standard library but eslint
                // disagrees.
                let valueSet = new Set();  // eslint-disable-line no-undef
...
function sanitizeEvent(event) {
    ...
    event.data.styles = !!event.data.styles ? event.data.styles : [];
    return event;
}

// eslint-disable-next-line no-unused-vars
var CodingGameController = new Lang.Class({
    ...
    _init: function(descriptors, service, chatController, effectsManager, logFile) {
        ...
        this._service.connectHandlers({
            ...
            startMission: Lang.bind(this, this._startMission)
        });
        ...
        this._dispatchTable = {
            ...
            'chat-separator': Lang.bind(this, this._dispatchChatSeparatorEvent),
    ...
    _dispatchChatSeparatorEvent: function(event, callback) {
        let entry = callback(event);
        this._chatController.sendChatMessage({
            timestamp: entry.timestamp,
            actor: entry.data.actor,
    ...
    _startMission: function(name) {
        ...
        // Dispatch a chat-separator event so that the chatbox knows
        // to display some kind of visual indicator that this is a new
        // task
        this._dispatch({
            // Currently hardcoded to Ada'
            name: 'separator::ada::' + name,
            type: 'chat-separator',
            data: {
                actor: 'Ada',
                separator: true
    ...
    _completeWaitForEventIn: function(event, timeout) {
        let afterTimeout = Lang.bind(this, function() {
            this._dispatch({
    ...
    _dispatch: function(event) {
        ...
        try {
            this._dispatchTable[sanitizedEvent.type](sanitizedEvent, Lang.bind(this, function(logEvent) {
