...
var DBUSService = new Lang.Class({
    Name: 'DBUSService',
    ...
    vfunc_handle_start_mission: function(method, mission) {
        try {
            this._handlers.startMission(mission);
            this.complete_start_mission(method);
        } catch (e) {
            logError(e, 'Could not start mission ' + mission);
            method.return_error_literal(CodingGameServiceErrorDomain,
                                        Errors.INTERNAL_ERROR,
                                        String(e));
        }
