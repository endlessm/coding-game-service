...
function copySourceToTarget(source, target) {
    // We need to explicitly use the local vfs here as EknVfs
    // interferes with normal operation of GFile.parse_name:
    //
    // https://phabricator.endlessm.com/T23125
    let targetFile = Gio.Vfs.get_local().parse_name(target);
    ...
    if (targetFile.has_prefix(Gio.File.new_for_path(GLib.get_home_dir()))) {
        ...
        executeCommandForOutput([
            ...
            '--host',
            'pkexec',
            '/usr/share/com.endlessm.CodingGameService/coding-copy-files',
            source,
            target
        ]);
    }
